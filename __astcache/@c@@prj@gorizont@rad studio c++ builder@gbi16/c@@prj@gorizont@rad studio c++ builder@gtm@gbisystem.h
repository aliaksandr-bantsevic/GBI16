...
public:
	...
	int ImportFromDataFile_GTM(TCHAR* path);
	int AcceptDataFileMeas_GTM(data_file_meas_gtm_type* dfm_gtm);
	TDrill* GetDrillByName(TPlace* p, WideString name);

	TMeas* selected_meas_list [SELECTED_MEAS_MAX];
	int selected_meas_list_max_len;
   	int selected_meas_list_idx;

	void GetSelectedMeas(void);
	int ViewSelectedMeas(TListView* list);

	bool system_meas_selected;
	void ReverseMeasSelection();

	int ExportComplexDataToCSV();
	int TGBISystem::ExportComplexDataToExl();
    WideString GetMeasHeader(int idx);
};
