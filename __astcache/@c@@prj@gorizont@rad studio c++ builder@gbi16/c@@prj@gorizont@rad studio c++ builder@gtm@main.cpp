...
TMeasChartThread* Tc_m = NULL;
TComplexChartThread* Tc_Complex = NULL;
...
bool b_last_meas_selected_state = false;

bool b_lshift_active = false;
bool b_lctrl_active = false;

HWND hMainWin;
HWND hChartWin;

int i_screen_Width = 1920;
int i_screen_Heigh = 1080;
int i_report_create_cnt;
//---------------------------------------------------------------------------
...
GBISystem->GetSelectedMeas();

Form_wait_report_create->Setup(GBISystem->selected_meas_list_max_len);

//ToolButton_testClick(NULL);

i_report_create_cnt = 0;

Timer_wait_report_create->Enabled = true;

return//!!!;

Tc_Complex->Draw();

TListView* list = ListView_complex_table;
//GBISystem->ViewSelectedMeas(list);

list = ListView_complex_data;
GBISystem->ViewSelectedMeas(list);

this->MPageControl->ActivePage = TabSheet_summary;

Timer_wait_report_create->Enabled = false;
Form_wait_report_create->Hide();
...
void TFMain::SystemInit()
{
	...
	WideString stitle(L"");
	stitle.printf(L"GTM [%s]" ,GBISystem->SysConfMgr.GetCurIniPath());
	this->Caption = stitle;
	...
	Tc_Complex->meas_list =  GBISystem->selected_meas_list;

	//hMainWin = GetActiveWindow();
}
...
void TFMain::ApplicationInit()
{
	...
	Tc_m = new TMeasChartThread (false, Chart_meas);
	//Tc_Complex = new TComplexChartThread (false, GBISystem->selected_meas_list, Chart_complex);
	Tc_Complex = new TComplexChartThread (false, NULL, Chart_complex);
	...
	this->TabSheet_meas_control->TabVisible = false;

	this->TabSheet_chart->TabVisible = false;
	this->TabSheet_report->TabVisible = false;
...
void __fastcall TFMain::N_add_placeClick(TObject *Sender)
{
	...
	TPlace* place = new TPlace(L"Новый объект");
...
void __fastcall TFMain::TreeView_systemDblClick(TObject *Sender)
{
	...
	if (GBISystem->CheckTreeItemSelected(&type, &idx, &obj) == 0)
	{
		...
		if (type == TREE_ITEM_TYPE_MEAS) {
			...
			current_meas = (TMeas*)obj;
			selected_meas = current_meas;

			current_meas_idx = idx;

			selected_meas->selected = b_last_meas_selected_state;
			selected_meas->ViewSelection();
...
void __fastcall TFMain::N_drill_addClick(TObject *Sender)
{
	...
	Form_ObjAdjust->set(0, d, &OK);
	Form_ObjAdjust->ShowModal();
...
void __fastcall TFMain::N_drill_adjustClick(TObject *Sender)
{
	...
	if (current_drill == NULL)
	{
		...
		ShowMessage(L"Ошибка выбора термоскважины!");
		return;
	}
...
void __fastcall TFMain::N_drill_deleteClick(TObject *Sender)
{
	...
	s.printf(L"Удалить термоскважину [%s] ?",d->name.c_bstr());
...
void __fastcall TFMain::TreeView_systemClick(TObject *Sender)
{
	...
	if (GBISystem->CheckTreeItemSelected(&type, &idx, &obj) == 0)
	{
		...
		if (type == TREE_ITEM_TYPE_SYSTEM) {
			...
			//if (b_lshift_active) GBISystem->ReverseMeasSelection();
			if (b_lctrl_active) GBISystem->ReverseMeasSelection();
		}
		...
		if (type == TREE_ITEM_TYPE_PLACE) {
			...
			if (b_lctrl_active) current_place->ReverseMeasSelection();
		}
		...
		if (type == TREE_ITEM_TYPE_DRILL) {
			...
			if (b_lctrl_active) current_drill->ReverseMeasSelection();
		...
		if (type == TREE_ITEM_TYPE_MEAS) {
			...
			b_last_meas_selected_state = current_meas->selected;

			if (b_lctrl_active) current_meas->ReverseSelect();
...
void TFMain::ViewSelectedMeas(void)
{
	...
	//if (b_lshift_active) selected_meas->fill_test_data();

	selected_meas->view_to_table(ListView_meas);
	Tc_m->Set(selected_meas);
    Tc_m->DrawChart();
...
void TFMain::DevideScreen(int par)
{
		...
		i_table_col_width = (this->Panel_meas_table->Width/14)+3;
			...
			else
			{
				...
				this->Panel_work_area->Height = Vertres*700/1000;
...
void __fastcall TFMain::ToolButton12Click(TObject *Sender)
{
	...
	if (this->GBISystem->selected_meas_list_max_len == 0)
	{
		utils_ShowMessage(L"Создайте отчет!");
		return;
	}

	this->MStatusBar->Panels->Items[2]->Text = L"Экспорт в Excel ...";

	//this->GBISystem->ExportComplexDataToCSV();
	this->GBISystem->ExportComplexDataToExl();
...
void __fastcall TFMain::MManualClick(TObject *Sender)
{
	TCHAR dir[1024];
	GetCurrentDirectoryW(1024,dir);
	ShellExecuteW(NULL,NULL,L"GTM.pdf",L"",dir,0);
}
//---------------------------------------------------------------------------
...
void __fastcall TFMain::TreeView_systemKeyDown(TObject *Sender, WORD &Key, TShiftState Shift)

{
	   if (Shift.Contains(ssShift)) b_lshift_active = true;
	   if (Shift.Contains(ssCtrl)) b_lctrl_active = true;
}
//---------------------------------------------------------------------------

void __fastcall TFMain::TreeView_systemKeyUp(TObject *Sender, WORD &Key, TShiftState Shift)

{
	   b_lshift_active = false;
	   b_lctrl_active = false;
}
//---------------------------------------------------------------------------
void TFMain::ShowChartHint_t(TChart* chart, int X, int Y)
{
			 bool sel = false;

			 for (int i = 0; i < GBISystem->selected_meas_list_idx; i++)
			 {

				 if ((chart->Series[i]->Clicked(X,Y)!=-1)||(b_lshift_active))
				 {
				   ...
				   //Screen->Cursor=crDrag;	//crCross;
					...
					ix = FMain->Left + Panel_tree->Width + Panel_summary_table->Width + 20 + X + 5;
					iy = FMain->Top + MToolBar->Height + 55 + 10 + Y + 5;

					if ((ix + 250) > i_screen_Width)
					{
						ix -= 250 + 5;
					}

					if ((iy + 80) > i_screen_Heigh)
					{
						iy -= 80 + 5;
					}

					Form_show_hint->Show(s);
					Form_show_hint->Move(ix,iy);
...
//---------------------------------------------------------------------------
void __fastcall TFMain::Chart_complexMouseMove(TObject *Sender, TShiftState Shift,
          int X, int Y)
{
	ShowChartHint_t(Chart_complex, X, Y);
}
//---------------------------------------------------------------------------

void __fastcall TFMain::ListView_complex_dataMouseMove(TObject *Sender, TShiftState Shift,
          int X, int Y)
{
  ...
  hti.pt.x=X;
  hti.pt.y=Y;
  //if (ListView_SubItemHitTest(ListView_complex_data->Handle,&hti)==-1) ShowMessage("Out of row and column");
  if (ListView_SubItemHitTest(ListView_complex_data->Handle,&hti)!=-1)
  //else
  {
		//if (b_lshift_active) ShowMessage("row= "+IntToStr(hti.iItem)+",   column= "+IntToStr(hti.iSubItem));
		ListView_complex_data->Hint = "row= "+IntToStr(hti.iItem)+",   column= "+IntToStr(hti.iSubItem);
		ListView_complex_data->ShowHint = true;

		s = ListView_complex_data->Hint;

		if ((hti.iItem >= 0) && (hti.iSubItem > 0))
		{
			int ii = hti.iSubItem - 1;
			WideString pname;
			WideString dname;
			WideString tmeas;
			...
			m = this->GBISystem->selected_meas_list [ii]; Sleep(10);
			p = this->GBISystem->place_list[m->pnum-1]; Sleep(10);
			d = p->drill_list[m->dnum-1]; Sleep(10);

			if ((m != NULL)&&(d != NULL)&&(p != NULL))
			{
				pname = p->name;
				dname = d->name;
				tmeas = FormatDateTime(L"dd.mm.yy hh.mm",m->create_time);
				...
				Form_show_hint->Show(s);
					...
					ix = FMain->Left + Panel_tree->Width + 20 + X + 5;
					iy = FMain->Top + MToolBar->Height + 55 + 10 + Y + 5;
...
/*
	 TPoint pt;
	 TListItem *pListItem = ListView_complex_data->GetItemAt(X, Y);
	 TListColumn *pListColumn = ListView_complex_data->GetColumnFromIndex(
	 //GetColumnAt(X, Y);
	 //if(pListItem)
	 if(pListColumn)
	 {
		ListView_complex_data->Hint = L"---------------------";//pListItem->Caption;
		ListView_complex_data->ShowHint = true;
	 }
	 else
	 {
		//ListView_complex_data->ShowHint = false;
	 }
*/
}
//---------------------------------------------------------------------------
...
void __fastcall TFMain::Timer_start_main_wndTimer(TObject *Sender)
{
		Timer_start_main_wnd->Enabled = false;
		hMainWin = GetActiveWindow();
}
//---------------------------------------------------------------------------
...
{
	Form_wait_report_create->Setup(20);
}
//---------------------------------------------------------------------------

void __fastcall TFMain::Timer_wait_report_createTimer(TObject *Sender)
{
/*
	Form_wait_report_create->Update(i_report_create_cnt);

	if (i_report_create_cnt == GBISystem->selected_meas_list_max_len)
	{
		Timer_wait_report_create->Enabled = false;
	}
*/
