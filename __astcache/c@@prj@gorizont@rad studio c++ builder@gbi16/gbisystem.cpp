 ...
 */
 int TGBISystem::AcceptDataFileMeas(data_file_meas_type* dfm)
 {
	 ...
	 {
		console(L"Система", L"НЕвалидное измерение пропущено" + st);
		return -1; //meas is not valued
	 }

	 if ((dfm->is_forward_valued == true)&&(dfm->is_back_valued == false))
	 {
		console(L"Система", L"Частично (F) валидное измерение обрабатывается" + st);
	 }

	 if ((dfm->is_forward_valued == false)&&(dfm->is_back_valued == true))
	 {
		console(L"Система", L"Частично (B) валидное измерение обрабатывается" + st);
	 }

	 if ((dfm->is_forward_valued == true)&&(dfm->is_back_valued == true))
	 {
		console(L"Система", L"Валидное измерение обрабатывается" + st);
	 }
	 ...
	 for (int i =0; i < dfm->record_cnt; i++)
	 {
		...
		{
			console(L"Система", L"НЕвалидная запись пропущена");
	...
	/* приведение к единому формату скважины */
	d->single_way = 1;
	d->start_point = DRILL_BOT_POINT; //start from bottom
