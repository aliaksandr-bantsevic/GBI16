...
int TDataFile::GetDaTaFileMeasIdx(void)
{
	...
	if (data_file_record.place == L"")
	{
		return -1;
	}

	if (data_file_record.drill == L"")
	{
		return -1;
	}
...
int TDataFile::session_correctness_check(void)
{
   ...
   data_file_meas_type* m = NULL;
   data_file_record_type* record;

   bool is_forward_start = false;
   bool is_forward_end = false;

   bool is_back_start = false;
   bool is_back_end = false;

   while ((data_file_meas_set[j] != NULL)&&(j<data_file_meas_set_idx))
   {
	  m = data_file_meas_set[j];

	  is_forward_start = false;
	  is_forward_end = false;
	  ...
	  m->is_valued = false;
	  m->is_forward_valued = false;
	  m->is_back_valued = false;

	  for (i = 0; i < m->record_cnt; i++)
	  {
		  record = &m->record [i];

		  record->is_sign_value = false;

		  if (record->dir == L"ForwardStart")
		  {
			   is_forward_start = true;
			   m->d_forward_start = record->level;
			   m->i_forward_start = i;
		  }

		  if (record->dir == L"BackStart")
		  {
			   is_back_start = true;
			   m->d_back_start = record->level;
			   m->i_back_start = i;
		  }

		  if (record->dir == L"ForwardEnd")
		  {
			   is_forward_end = true;
	  ...
	  if ((is_forward_start)&&(is_forward_end))
	  {
		  for (i = 0; i < m->record_cnt; i++)
		  {
			  ...
			  if ((i >= m->i_forward_start)&&(i <= m->i_forward_end))
			  {
				  record->is_sign_value = true;
			  }
		  }

		  m->is_forward_valued = true;
	  }

	  /* forward part is valid */
	  if ((is_back_start)&&(is_back_end))
	  {
		  for (i = 0; i < m->record_cnt; i++)
		  {
			  record = &m->record [i];

			  if ((i >= m->i_back_start)&&(i <= m->i_back_end))
			  {
				  record->is_sign_value = true;
			  }
		  }

		  m->is_back_valued = true;
	  }
